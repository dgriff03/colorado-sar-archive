...
export default function Home() {
  ...
  const [selected, setSelected] = useState<string | null>(null),
    [detail, setDetail] = useState<Incident | null>(null),
  ...
  function update(key: keyof Filters, value: string) {
    setFilters((f) => ({
      ...f,
      [key]: value,
      ...(key === 'q' ? { sort: value.trim() ? 'relevance' : 'newest' } : {}),
    }));
    setPage(1);
  }
  function clear() {
  ...
  useEffect(() => {
    ...
    if (id && /^(legacy-\d{6}|[0-9a-f-]{36})$/.test(id)) setSelected(id);
    setReady(true);
    return () => ctrl.abort();
  }, []);
  useEffect(() => {
    ...
    history.replaceState(null, '', p.size ? '/?' + p.toString() : '/');
    ...
    <>
      ...
      <main id="main-content" className="archive">
          ...
          <section className="results" aria-label="Incident search">
            ...
            {status === 'error' ? (
              ...
              <div className="incident-list">
                {results.slice((safePage - 1) * 20, safePage * 20).map((r) => (
                    ...
                    <div className="incident-main">
                      <div className="incident-tags">
                        <span
                          className={
                            'type-tag ' +
                            (r.incident_type === 'avalanche' ? 'amber' : '')
                          }
                        >
                      ...
                      <div className="incident-location">
                        <MapPin size={14} />
                        <span>
                          {r.location || r.place || 'Location not recorded'}
                        </span>
                      </div>
                    </div>
                    <button
        ...
        <footer className="site-footer">
          <span>Colorado SAR Archive</span>
          <p>

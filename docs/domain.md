# Canonical domain: accidents.typetwo.dev

The website, copied incident links, MCP result links, documentation and canonical
metadata use `https://accidents.typetwo.dev`. The hosted MCP URL is
`https://accidents.typetwo.dev/api/mcp`; the human-readable guide is `/mcp/`.
...
2. Enter `accidents.typetwo.dev` as a serving domain, not a redirect to another site.
...
- A copied incident URL starts with `https://accidents.typetwo.dev/` and reopens the same incident.
...
the application/MCP origin. Keep it and static documentation aligned if the domain
changes again. Relative website navigation works on either hostname.
...
`public/llms.txt` is a concise discovery guide. `public/data-guide.md` describes
fields and interpretation. `npm run docs:build` generates Markdown FAQ and MCP
pages from their existing sources, and runs automatically before site dev/build.
